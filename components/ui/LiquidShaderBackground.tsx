...
// ⚡ PERFORMANCE: Three.js (~500KB) é carregado LAZILY após o paint inicial.
// Importação estática `import * as THREE from 'three'` bloqueava o render por 4-6 segundos.
// Agora o WebGL inicia apenas após a landing page ser visível.
...
export const LiquidShaderBackground: React.FC<LiquidShaderBackgroundProps> = ({ intensity = 2.2 }) => {
    ...
    useEffect(() => {
        ...
        // Import dinâmico: Three.js só é baixado após o primeiro frame renderizado
        const initWebGL = async () => {
            try {
                // Aguardar um frame para garantir que a UI está visível antes de carregar 500KB
                ...
                class GradientBackground {
                    ...
                    constructor(sceneManager: any) {
                        ...
                        this.uniforms = {
                            ...
                            uColor1: { value: new THREE.Vector3(0.06, 0.72, 0.50) },
                            uColor2: { value: new THREE.Vector3(0.02, 0.30, 0.23) },
                            uColor3: { value: new THREE.Vector3(0.20, 0.82, 0.60) },
                            uColor4: { value: new THREE.Vector3(0.01, 0.15, 0.12) },
                            uColor5: { value: new THREE.Vector3(0.10, 0.40, 0.30) },
                            uColor6: { value: new THREE.Vector3(0.02, 0.10, 0.08) },
                            uSpeed: { value: 0.8 }, uIntensity: { value: intensity },
                            uTouchTexture: { value: null }, uGrainIntensity: { value: 0.05 },
                            uDarkNavy: { value: new THREE.Vector3(0.008, 0.015, 0.012) },
                ...
                appRef.current = new WebGLApp(container);
                if (!cancelled) setIsReady(true);

            } catch (err) {
                console.warn('[LiquidShaderBackground] WebGL não disponível, usando fundo estático.', err);
            }
        };
    ...
    useEffect(() => {
        if (appRef.current?.gradientBackground) {
    ...
    return (
        <div className="fixed inset-0 pointer-events-none z-0">
            {/* Fundo estático CSS enquanto o WebGL não carregou — elimina flash branco */}
            {!isReady && (
                <div
                    className="absolute inset-0 z-0"
                    style={{
                        background: 'radial-gradient(ellipse at 30% 40%, rgba(16,185,129,0.15) 0%, transparent 60%), radial-gradient(ellipse at 70% 60%, rgba(6,78,59,0.2) 0%, transparent 60%), #020202'
                    }}
                />
            )}
            <div
                ref={containerRef}
                className="absolute inset-0 z-0"
                style={{ pointerEvents: 'auto', opacity: isReady ? 1 : 0, transition: 'opacity 0.8s ease' }}
            />
