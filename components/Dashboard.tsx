...
const Dashboard: React.FC<DashboardProps> = ({ user, logs, onNavigate, onLogout, onDeleteLog, onEditLog, waterConsumed, setWaterConsumed, onShowToast }) => {
   ...
   return (
      <div className="max-w-[420px] mx-auto px-4 pt-4 md:pt-24 pb-32 md:pb-12 text-gray-900 dark:text-white min-h-screen">
         ...
         <div className="relative mb-8 md:mb-10 group px-1">
            {/* Outer Glow Decor */}
                  ...
                  <div className="w-full space-y-2">
                     <div className="flex justify-between items-center px-1">
                        <div className="flex gap-1">
                           {[...Array(10)].map((_, i) => (
                              <div key={i} className={`h-1 w-3 rounded-sm transition-all duration-500 ${visualPercent > (i * 10) ? (isOverLimit ? 'bg-red-500 shadow-[0_0_5px_red]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]') : 'bg-white/5'}`}></div>
                           ))}
                        </div>
         ...
         {/* HYDRATION CARD - VERTICAL OPTIMIZED */}
         <div className={`
            p-6 md:p-8 rounded-[2.5rem] mb-8 md:mb-8 relative overflow-hidden transition-all duration-500 border
            ${isWaterGoalMet
               ? 'bg-gradient-to-br from-blue-600 to-cyan-500 shadow-2xl shadow-cyan-500/40 border-cyan-400/50 scale-[1.02]'
         ...
         `}>
            ...
            <div className="flex flex-col items-center md:items-start text-center md:text-left mb-6 relative z-10">
               <div className="flex flex-col md:flex-row items-center gap-3 w-full">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-inner transition-transform group-hover:scale-110 ${isWaterGoalMet ? 'bg-white/20 text-white' : 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600'}`}>
                     💧
                  </div>
                  <div className="flex flex-col flex-1 min-w-0">
                     <h3 className={`font-black text-xl leading-tight ${isWaterGoalMet ? 'text-white' : 'text-gray-900 dark:text-white'}`}>Hidratação</h3>
                     {manualWaterEdit ? (
                        <div className="flex items-center justify-center md:justify-start gap-2 mt-1">
                           <input
                              ...
                              className="w-20 bg-black/5 dark:bg-white/5 rounded-lg px-2 py-1 text-sm font-bold outline-none border border-cyan-500 text-center md:text-left"
                              autoFocus
                              onBlur={() => setManualWaterEdit(false)}
                           />
                           <span className={`text-xs font-bold ${isWaterGoalMet ? 'text-white/60' : 'text-gray-400'}`}>/ {waterGoal} ml</span>
                        </div>
                     ) : (
                        <span
                           className={`text-xs font-black uppercase tracking-widest mt-1 cursor-pointer hover:underline ${isWaterGoalMet ? 'text-cyan-100' : 'text-cyan-600 dark:text-cyan-400'}`}
                  ...
                  {isWaterGoalMet && (
                     <div className="hidden md:block bg-white/20 backdrop-blur-md px-3 py-1 rounded-full border border-white/20">
            ...
            <div className="w-full space-y-4 mb-6 relative z-10">
               <div className={`w-full h-4 rounded-full overflow-hidden border p-0.5 ${isWaterGoalMet ? 'bg-black/20 border-white/10' : 'bg-gray-100 dark:bg-zinc-800 border-black/5 dark:border-white/5'}`}>
                  <div
            ...
            <div className="flex gap-3 relative z-10">
               <button
                  ...
                  className={`flex-1 py-4 rounded-2xl font-black uppercase text-sm tracking-[0.1em] active:scale-95 transition-all shadow-xl flex items-center justify-center gap-2 ${isWaterGoalMet ? 'bg-white text-blue-600 hover:bg-cyan-50' : 'bg-cyan-500 text-white hover:bg-cyan-600 shadow-cyan-500/30'}`}
               >
                  <span className="text-xl">+</span> 250ml
               </button>
               <button
                  onClick={() => setShowWaterShortcuts(!showWaterShortcuts)}
                  className={`w-14 flex items-center justify-center rounded-2xl border-2 transition-all ${isWaterGoalMet
               ...
               >
                  <svg className={`w-6 h-6 transition-transform duration-300 ${showWaterShortcuts ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M19 9l-7 7-7-7" /></svg>
               </button>
            </div>
         ...
         {/* MACRO GRIDS - FULL VERTICAL ON MOBILE */}
         < div className="flex flex-col md:grid md:grid-cols-3 gap-3 md:gap-4 mb-8 md:mb-10" >
...
const MacroCard = ({ label, value, unit, fullLabel, goal, color = 'emerald' }: any) => {
   ...
   return (
      <div className="glass-panel p-4 md:p-5 rounded-2xl md:rounded-[2rem] flex flex-col md:items-center md:justify-center hover:-translate-y-1 transition-all duration-300 group shadow-premium hover:shadow-premium-hover min-w-0">
...
const ActionButton = ({ onClick, icon, title, subtitle, premium }: any) => (
   <button onClick={onClick} className="glass-panel w-full p-5 md:p-6 rounded-3xl md:rounded-[2rem] text-center md:text-left hover:bg-white/80 dark:hover:bg-zinc-800/80 hover:scale-[1.02] transition-all duration-300 relative overflow-hidden group border border-transparent hover:border-emerald-500/30 shadow-premium hover:shadow-premium-hover h-full flex flex-col items-center md:items-start justify-center md:justify-between min-w-0">
      ...
      <div className="min-w-0 w-full flex flex-col items-center md:items-start">
         <h3 className="font-black text-sm md:text-base leading-tight text-gray-900 dark:text-white mb-1 w-full truncate">{title}</h3>
         {subtitle && <p className="text-[11px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-tight w-full truncate">{subtitle}</p>}
      </div>
   </button>
);
