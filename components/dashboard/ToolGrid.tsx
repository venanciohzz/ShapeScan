...
const ActionButton: React.FC<ActionButtonProps> = ({ onClick, icon, title, subtitle, highlight }) => (
    <motion.button
        ...
        onClick={onClick}
        className={`relative overflow-hidden group backdrop-blur-2xl p-4 md:p-8 rounded-[1.5rem] md:rounded-[2rem] transition-all duration-500 shadow-xl flex flex-col items-center md:items-start text-center md:text-left h-full ${
            highlight
    ...
    >
        <div className="relative z-10 w-full flex flex-col items-center md:items-start">
            <div className="flex justify-center md:justify-between items-start mb-3 md:mb-10 w-full">
                <div className={`transition-all duration-500 ${highlight ? 'text-emerald-400' : 'text-zinc-300 group-hover:text-emerald-400'}`}>
                    {icon}
                </div>
            </div>
            <div className="flex flex-col items-center md:items-start">
                <h3 className="font-serif-premium font-bold text-sm md:text-xl text-white mb-1 md:mb-2 tracking-tight group-hover:text-emerald-400 transition-colors leading-tight">
                    {title}
                </h3>
                {subtitle && (
                    <p className="text-[9px] md:text-[10px] font-black text-zinc-400 uppercase tracking-widest opacity-80">
                        {subtitle}
                    </p>
                )}
            </div>
...
const PremiumButton: React.FC<PremiumButtonProps> = ({ onClick, icon, title, subtitle }) => (
    <motion.button
        ...
        transition={{ type: "spring", stiffness: 400, damping: 17 }}
        onClick={onClick}
        className="relative overflow-hidden group bg-zinc-900/40 border border-white/5 backdrop-blur-2xl p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] hover:bg-zinc-800/50 hover:border-emerald-500/20 transition-all duration-500 shadow-xl flex flex-col items-center md:items-start text-center md:text-left h-full"
    >
        <div className="relative z-10 w-full flex flex-col items-center md:items-start">
            <div className="flex justify-between items-start mb-3 md:mb-4 w-full">
                <div className="text-zinc-500 group-hover:text-emerald-400/70 transition-all duration-500">
                    {icon}
                </div>
                <div className="bg-emerald-500/10 text-emerald-400/70 px-1.5 py-0.5 rounded-md border border-emerald-500/20 text-[7px] font-black uppercase tracking-widest">
                    Pro
                </div>
            </div>
            <div className="flex flex-col items-center md:items-start">
                <h3 className="font-serif-premium font-bold text-sm md:text-lg text-zinc-400 group-hover:text-zinc-200 mb-0.5 md:mb-1 tracking-tight transition-colors leading-tight">
                    {title}
                </h3>
                {subtitle && (
                    <p className="text-[8px] md:text-[9px] font-black text-zinc-600 uppercase tracking-widest">
                        {subtitle}
                    </p>
                )}
            </div>
