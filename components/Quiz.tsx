...
const Quiz: React.FC<QuizProps> = ({ onComplete, isLoading = false }) => {
      ...
      <div className="glass-panel w-full max-w-md p-6 md:p-10 rounded-[2.5rem] shadow-2xl relative border border-white/20 my-auto">
        ...
        {step === 4 && (
          <div className="animate-in fade-in slide-in-from-right duration-300">
            ...
            <div className="flex gap-4">
              <button onClick={prevStep} disabled={isLoading} className="flex-1 py-5 bg-transparent border-2 border-gray-200 dark:border-white/10 text-gray-500 dark:text-zinc-400 rounded-2xl font-black uppercase tracking-widest hover:bg-black/5 disabled:opacity-50">Voltar</button>
              <button onClick={handleSubmit} disabled={isLoading} className="flex-[2] bg-emerald-600 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-lg shadow-emerald-500/20 hover:bg-emerald-500 disabled:opacity-70 flex items-center justify-center gap-2">
                {isLoading ? (
                  <>
